...
When triage is done, show the user the finalized review — the recommendation and the surviving findings — and gate on an explicit confirmation before anything is posted. Be clear about what posting does: it lands **one review** on the PR, with each surviving finding as its own inline comment on its line and the recommendation as the summary — not everything crammed into a single comment. On confirmation, hand the finalized review to the `pull-request-commenter`, which posts it through the `/review` skill's poster. You do not post directly; posting is the commenter's mechanical job.
