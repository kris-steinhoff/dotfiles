...
When triage is done, show the user the finalized review — the recommendation and the surviving findings — and gate on an explicit confirmation before anything is posted. Be clear about what posting does: it lands **one review** on the PR, with each surviving finding as its own inline comment on its line and the recommendation as the summary — not everything crammed into a single comment. On confirmation, hand the finalized review to the `commenter`, which lands it on the PR through the `post-pr-review` skill. You do not post directly; posting is the commenter's mechanical job. Do **not** compose or pass an attribution footer in the hand-off — the commenter owns attribution and always uses the bare `_Posted by {agent_name} on behalf of {user_full_name}._` (the agent name alone, no model). Dictating a footer here is exactly what put a model name ("Claude (Opus 5)", "Claude Haiku 4.5") on posted reviews.
