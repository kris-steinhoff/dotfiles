...
3. **Change Summaries**: Use the `/change-summaries` format when summarizing changes.
4. **Markdown Formatting**: Use the `/markdown-formatting` standard when editing markdown files.
5. **Pre-commit Rules**: Use the `/pre-commit` rules when running formatters or linters.
