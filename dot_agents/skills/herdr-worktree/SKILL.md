---
name: herdr-worktree
description: "Create a Herdr worktree workspace, either checking out a GitHub pull request's branch or making a worktree for a fresh branch. Use when the user asks to open a PR locally, get a PR's code on disk, check out a branch in a worktree, or spin up a new-branch worktree without disturbing the current checkout. Requires HERDR_ENV=1."
---

# Herdr worktree

Puts a branch in its own worktree, opened as a Herdr workspace, without touching the current checkout. Either an existing pull request's branch, or a fresh branch you name.

This skill assumes the `herdr` skill for CLI discovery, ID handling, and the safety rules. It does not repeat them.

It creates the worktree and stops. It does not start an agent — once it returns, start one in the worktree's pane with the `herdr` skill's `agent start`. The judgement that turns "have <agent> work on #123" into a task lookup, a branch name, this skill, and an agent lives in the always-loaded Herdr delegation instructions.
...
## `pr` — an existing pull request

```bash
scripts/worktree pr [<pr>] [--path <path>] [--no-focus] [--no-update] [--vacate]
```

With no argument it takes the current branch's pull request. It checks `HERDR_ENV`, resolves the PR, fetches the head, and creates the worktree. The local branch keeps the PR's own branch name. No `pr-123` renaming, so the branch reads the same locally as it does on GitHub and in the PR page.
...
```bash
scripts/worktree create --branch <name> [--label <label>] [--base <ref>] [--path <path>] [--no-focus]
```

Makes a worktree for `--branch`. A branch that does not exist yet is created from `--base` (the repo HEAD by default); an existing local branch is checked out as it stands. `--label` defaults to the branch name.
...
This subcommand does not fetch a task or invent a branch name. Deciding the branch and slug is judgement, and it lives in the always-loaded Herdr delegation instructions. Call `create` once you already have the name.
...
**Re-running converges.** An existing worktree on the branch is opened and focused (`"reused": true`) rather than recreated, so starting the same issue twice lands in the same place.

## Focus

**Focus is on by default**, since the user asked to open the branch. Pass `--no-focus` when staging several worktrees at once, or when the user is mid-task and said not to move them.

## Arriving somewhere that already exists (`pr`)

Running `pr` twice on the same PR is normal, so the script is written to converge rather than to fail. It never needs a manual pull or a manual focus afterwards.
