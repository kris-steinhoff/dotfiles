...
The `chief-of-staff` is a different kind of persona from the other five: its unit of work is a **commitment** tracked over weeks, not a session of code work, so it holds persistent state rather than delegating a bounded task. It keeps that state as an [Open Knowledge Format](https://cloud.google.com/blog/products/data-analytics/how-the-open-knowledge-format-can-improve-data-sharing/) (OKF) bundle — a directory of markdown files, one concept per file, linked into a graph — at `~/.local/state/chief-of-staff/`: `commitments/` (what the user owes people and what people owe the user, `type` distinguishing the direction), `in-flight/` (dispatched work), `decisions/` (worth not relitigating), and `people/` (who they are), with closed items moved to `archive/`. Each file's path is its identity and its frontmatter carries the queryable fields (`type`, plus dates, source, tags); an owed item links to the person it's owed to, so "what do I owe Sarah" is a graph traversal rather than a scan. A maintained `index.md` is the roll-up read at session start — one line per open item, each linking to its file — so opening a session loads the state at a glance without reading every concept. Its rule is **dispatch and record, never relay**: it can launch a coordinator (into its own pane or worktree, where that coordinator talks to the user directly) and it writes down that it dispatched, but it never channels a worker's output back through itself, because it is the tier with the least direct evidence. It does not supervise or override the coordinator. It is **read-and-draft only and takes no action out to the world**: it maintains the bundle and drafts what the user then sends (a brief, a nudge on a stale waiting-on), but it never sends a message, contacts anyone, or fires on a schedule — the brief is produced on demand, there is no send authority, and no trust ramp to climb. Its tools are left open (it needs `Write`/`Edit` for the bundle and Bash for dispatch) and that boundary is prose, not a tool lock. Claude and Codex both wrap the canonical prompt: Codex exposes it as `codex --profile chief-of-staff` and as a spawnable `[agents.chief-of-staff]` role, with workspace access extended to the global state bundle. Connector-derived state remains deferred, and there is no Gemini wiring yet.

The bundle is live state the persona rewrites constantly, so it must not be clobbered on every `chezmoi apply` the way a plain template would. The seed `dot_local/state/chief-of-staff/create_index.md` uses chezmoi's `create_` attribute: the target is written once on a fresh machine and left untouched on every apply after, so a populated bundle survives. Only the `index.md` is seeded — it documents the layout in a comment and carries the empty section headings; the concept subdirectories (`commitments/`, `people/`, …) are pure runtime state the persona creates as items arise, unmanaged by chezmoi. Real entries are hand-seeded or accrue in use, since example lines would become stale live data. This is the same machine-local-mutable-state problem the Codex `config.toml` and Claude `settings.json` handling solves, reached here through `create_` rather than a `modify_` script because a seed-once-then-hands-off file needs no round-trip merge. The state moved out of its first home at `~/.claude/cos/` (a `ledger.md` + `archive.md` pair) into `~/.local/state/`, the XDG state dir; `.chezmoiremove` deletes the old `~/.claude/cos/` so the retired seeds don't linger on machines that already applied them.
